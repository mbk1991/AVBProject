    ...
    <div id="contents">
        <div id="wrap">
            <form action="/member/nickname/register.do" method="post">
                <input type="text" id="id"  name="memberId" placeholder="아이디를 입력하세요." onblur="idCheck(this);" required>
                <div id="idCheck"></div>
                <input type="password" id="pwd" name="memberPwd"  placeholder="비밀번호를 입력하세요." onblur="pwdCheck(this);" required>
                <input type="password" id="rePwd" placeholder="비밀번호를 재입력하세요." onblur="rePwdCheck(this)"  required>
                <div id="pwdCheck"></div>
                <input type="text" id="nickname" name="nickName"  readonly placeholder="클릭하세요>>"> 
...
function pwdCheck(thisInput){
	var regExPwd = /^[0-9a-zA-Z]{4,12}$/;
		if(!regExPwd.test(thisInput.value) && (thisInput.value != "")){
			document.querySelector("#pwdCheck").innerHTML="<span style='color:red'>6~16자 영문,숫자로 입력하세요.</span>";
			thisInput.value = "";
		}else{
